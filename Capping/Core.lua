		...
		do
			local function ReportBar(bar, channel)
				...
				if not custom then
					...
					msg = format("Midnight Capping: %s capping in %s!", bar:GetLabel(), timeLeft)
				else
					msg = custom(bar)
				end
				if msg then
	...
	if addon == addonName then
		...
		local defaults = {
			profile = {
				...
				alignTime = "RIGHT",
				alignIcon = "LEFT",
				colorText = {1, 1, 1, 1}, -- Pure White
				colorAlliance = {0.7, 0.5, 1.0, 1.0}, -- Ethereal Silver/Purple
				colorHorde = {0.4, 0.1, 0.8, 1.0}, -- Bright Void Purple
				colorQueue = {0.6, 0.6, 0.6, 1}, -- Neutral Grey
				colorOther = {0.5, 0.0, 0.5, 1.0}, -- Deep Purple
				colorBarBackground = {0.05, 0.02, 0.1, 0.9}, -- Void Black
				minimapAngle = 220,
				minimapIcon = true,
				queueBars = true,
		...
		header:SetText("Midnight Capping")
		frame.header = header
		...
		function core:LOADING_SCREEN_DISABLED()
			...
			self:Timer(0, function() -- Timers aren't fully functional until 1 frame after loading is done
				self:Timer(15, function()
   					print("|cFF9966FFMidnight Capping|r is missing locale for ".. needsLocale[loc] .." and needs your help! Please visit the project page on GitHub for more info.")
				end)
			end)
		end
...
-- MINIMAP BUTTON
do
	local minimapButton = CreateFrame("Button", "MidnightCappingMinimapButton", Minimap)
	...
	icon:SetSize(21, 21)
	icon:SetTexture(134269) -- Interface/Icons/INV_Misc_Map02 (map scroll icon)
	icon:SetPoint("CENTER", 0, 1)

	local isDragging = false
	local function UpdatePosition()
		local angle = db.profile.minimapAngle or 220
		local rad = math.rad(angle)
		local x = math.cos(rad) * 104
		local y = math.sin(rad) * 104
	...
	minimapButton:SetScript("OnDragStart", function()
		...
		minimapButton:SetScript("OnUpdate", function()
			...
			db.profile.minimapAngle = angle
			minimapButton:ClearAllPoints()
			local rad = math.rad(angle)
			minimapButton:SetPoint("CENTER", Minimap, "CENTER", math.cos(rad) * 104, math.sin(rad) * 104)
		end)
	end)
	...
	minimapButton:SetScript("OnClick", function(_, btn)
		if btn == "LeftButton" then
			C_AddOns.EnableAddOn("Capping_Options")
	...
	local tooltip = CreateFrame("GameTooltip", "MidnightCappingMinimapTooltip", UIParent, "GameTooltipTemplate")
	minimapButton:SetScript("OnEnter", function(self)
		if isDragging then return end
		tooltip:ClearLines()
		tooltip:SetOwner(self, "ANCHOR_LEFT")
		tooltip:AddLine("|cFF9966FFMidnight Capping|r")
		tooltip:AddLine("|cFFFFFFFFLeft-click|r to open options", 0.8, 0.8, 0.8)
		tooltip:AddLine("|cFFFFFFFFDrag|r to move around minimap", 0.8, 0.8, 0.8)
		tooltip:Show()
	end)
	minimapButton:SetScript("OnLeave", function() tooltip:Hide() end)

	-- Position after DB is loaded; poll until db is available
	minimapButton:Hide()
	...
	initFrame:SetScript("OnEvent", function(self)
		...
		if db and db.profile then
			UpdatePosition()
			if db.profile.minimapIcon ~= false then
				minimapButton:Show()
			end
		end
	end)
...
do
 ...
 SlashCmdList.MidnightCapping = openOpts
 SLASH_MidnightCapping1 = "/mcap"
